...
import { AIContextBuilder, AIRecommendationsService, FakeAIProvider } from '../src/intelligence';
...
async function main(): Promise<void> {
    ...
    recommendations: [
      {
        ...
        action:
          'Compare version 42 with the previous known-good checkout version and inspect the deployment changes.',
        priority: 'HIGH',
        confidence: {
          level: 'MEDIUM',
          score: 0.72,
      ...
      },
      {
        ...
        action:
          'Review database timeout metrics and connection behavior during the incident window.',
        priority: 'IMMEDIATE',
        confidence: {
          level: 'HIGH',
          score: 0.84,
          rationale:
            'The timeout signal is directly represented in supplied event and evidence data.',
        },
        references: [
          {
            type: 'EVENT',
  ...
  assert.match(result.recommendations[0].id, /^recommendation-[a-f0-9]{24}$/);

  assert.deepEqual(
    result.recommendations[1].references.map((reference) => `${reference.type}:${reference.id}`),
  ...
  const malformedService = new AIRecommendationsService(malformedProvider, builder);
